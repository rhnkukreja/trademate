...
HISTORICAL_DAYS = 300  # Days of historical data to fetch for daily analysis
HOURLY_HISTORICAL_DAYS = 50  # Days to fetch for hourly analysis (to get 200+ hourly candles)
...
def calculate_daily_moving_averages(data):
    """Calculate daily moving averages from historical data"""
    if not data or len(data) < 200:
        logger.warning(f"Insufficient daily data: {len(data) if data else 0} candles, need 200+")
        return None

    df = pd.DataFrame(data)
    df['date'] = pd.to_datetime(df['date']).dt.date
    df = df.sort_values('date').tail(200).reset_index(drop=True)  # Get last 200 days
...
def calculate_hourly_moving_averages(data):
    """Calculate hourly moving averages from historical data"""
    if not data or len(data) < 200:
        logger.warning(f"Insufficient hourly data: {len(data) if data else 0} candles, need 200+")
        return None

    df = pd.DataFrame(data)
    df['datetime'] = pd.to_datetime(df['date'])
    df = df.sort_values('datetime').tail(200).reset_index(drop=True)  # Get last 200 hours

    # Calculate moving averages with different field names for hourly
    df['ma_44h'] = df['close'].rolling(window=44, min_periods=44).mean().round(2)
    df['ma_50h'] = df['close'].rolling(window=50, min_periods=50).mean().round(2)
    df['ma_100h'] = df['close'].rolling(window=100, min_periods=100).mean().round(2)
    df['ma_200h'] = df['close'].rolling(window=200, min_periods=200).mean().round(2)

    # Get latest values
    latest = df.tail(1).iloc[0]

    return {
        'ma_44h': None if pd.isna(latest['ma_44h']) else float(latest['ma_44h']),
        'ma_50h': None if pd.isna(latest['ma_50h']) else float(latest['ma_50h']),
        'ma_100h': None if pd.isna(latest['ma_100h']) else float(latest['ma_100h']),
        'ma_200h': None if pd.isna(latest['ma_200h']) else float(latest['ma_200h']),
        'current_price': float(latest['close']),
        'datetime': latest['datetime']
    }

def check_daily_cp_above_all_mas(stock_data):
    """Check if current price is above all 4 daily moving averages"""
    if not stock_data:
        return False
...
def check_hourly_cp_above_all_mas(stock_data):
    """Check if current price is above all 4 hourly moving averages"""
    if not stock_data:
        return False

    mas = ['ma_44h', 'ma_50h', 'ma_100h', 'ma_200h']
    # Check if all MAs are available and CP is above all of them
    for ma in mas:
        if stock_data[ma] is None or stock_data['current_price'] <= stock_data[ma]:
            return False
...
MA-44h: {hourly_current['ma_44h']:.2f}
MA-50h: {hourly_current['ma_50h']:.2f}
MA-100h: {hourly_current['ma_100h']:.2f}
MA-200h: {hourly_current['ma_200h']:.2f}
"""
...
def fetch_and_analyze_stocks():
    ...
    for i, instrument in enumerate(instruments, start=1):
        try:
            ...
            daily_from_date = target_date - timedelta(days=HISTORICAL_DAYS)
            ...
            hourly_from_date = datetime.now() - timedelta(days=HOURLY_HISTORICAL_DAYS)
            ...
            if daily_historical_data:
                daily_analysis = calculate_daily_moving_averages(daily_historical_data)

            # Calculate hourly moving averages
            hourly_analysis = None
            if hourly_historical_data:
                hourly_analysis = calculate_hourly_moving_averages(hourly_historical_data)
            ...
            if hourly_analysis:
                hourly_stock_summary = {
                    "company_name": company_name,
                    "ma_44h": hourly_analysis['ma_44h'],  # Now correctly matches expected field names
                    "ma_50h": hourly_analysis['ma_50h'],
                    "ma_100h": hourly_analysis['ma_100h'],
                    "ma_200h": hourly_analysis['ma_200h'],
                    "current_price": hourly_analysis['current_price'],
                    "updated_at": datetime.now().isoformat()
                }
                all_hourly_data.append(hourly_stock_summary)

            # Check for breakouts using separate functions
            daily_cp_above_all_mas = check_daily_cp_above_all_mas(daily_analysis) if daily_analysis else False
            hourly_cp_above_all_mas = check_hourly_cp_above_all_mas(hourly_analysis) if hourly_analysis else False
